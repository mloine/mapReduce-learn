...
/**
 * @Author mloine
 * @Description 使用MR 实现  select shape,count(shape) from table group by shape
 ...
 */
public class SelectShapeDriver extends Configured implements Tool {

    public static void main(String[] args) {
        try {
            System.exit(ToolRunner.run(new SelectShapeDriver(),args));
        }catch (Exception e){
            e.printStackTrace();
        }
    }
    ...
    public int run(String[] args) throws Exception {
        ...
        Job job = Job.getInstance(conf, "SelectShapeMR");
        ...
        //map class
        job.setMapperClass(SelectShapeMapper.class);
        job.setMapOutputKeyClass(Text.class);
        job.setMapOutputValueClass(IntWritable.class);

        //Combiner class 在map阶段先进行数据的本地聚合 （可选 用于本地聚合减少传输数据提高MR执行效率）
        job.setCombinerClass(SelectShapeReduce.class);

        //reduce class
        job.setReducerClass(SelectShapeReduce.class);
